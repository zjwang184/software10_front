<template>
  <div>
    ...
    <el-card class="user_list_card">
      ...
      >
        <el-table-column prop="label" label="病种名称" sortable width="300px">
        </el-table-column>
        <el-table-column prop="icdCode" label="icd编码" sortable>
        </el-table-column>
        <el-table-column prop="tableNum0" label="表数量(公共数据集)" sortable>
        </el-table-column>
        <el-table-column prop="tableNum1" label="表数量(私有数据集)" sortable>
        </el-table-column>
        <el-table-column prop="tableNum2" label="表数量(共享数据集)" sortable>
        </el-table-column>
        <el-table-column prop="username" label="创建者" sortable>
        </el-table-column>
        <el-table-column
          label="操作（注释：删除上级病种时会将该病种下所有病种删除！）"
          width="400"
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              icon="el-icon-edit"
              circle
              @click="getInfoDisease(scope.row)"
            ></el-button>
            <el-popconfirm
              ...
              icon-color="red"
              title="确定删除该病种吗？"
              @confirm="deleteDisease(scope.row)"
            >
              <el-button
                type="danger"
                icon="el-icon-delete"
    ...
    >
      <el-form
        ref="form"
        :model="addDiseaseParam"
        :label-position="labelPosition"
      >
        <el-form-item label="病种名称">
          <el-input
            v-model="addDiseaseParam.firstDisease"
            placeholder="请输入病种名称"
            style="width: 300px"
            @blur="checkDiseaseName(addDiseaseParam.firstDisease)"
          ></el-input>
        </el-form-item>
        <el-form-item label="病种icd编码">
          <el-input
            v-model="addDiseaseParam.icdCode"
            placeholder="请输入病种icd编码"
            style="width: 300px"
          ></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog
      title="编辑病种"
      :visible.sync="editDialogDiseaseVisible"
      :close-on-click-modal="false"
    >
      <el-form
        ref="form"
        :model="updateDiseaseParam"
        :label-position="labelPosition"
      >
        ...
        <el-form-item label="病种名称" style="width: 228px">
          <el-input
            v-model="updateDiseaseParam.diseaseName"
            :placeholder="updateDiseaseParam.diseaseName"
            style="width: 300px"
            @blur="checkDiseaseName(updateDiseaseParam.diseaseName)"
          ></el-input>
        </el-form-item>
        <el-form-item label="病种icd编码" style="width: 228px">
          <el-input
            v-model="updateDiseaseParam.icdCode"
            :placeholder="updateDiseaseParam.icdCode"
            style="width: 300px"
          ></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
      ...
      getCategory("/api/category/getAllDisease").then((response) => {
        ...
        for (var i = 0; i < this.tableData.length; i++) {
          ...
          for (var j = 0; j < this.tableData[i].children.length; j++) {
            let a = {
              value: this.tableData[i].children[j].id,
              label: this.tableData[i].children[j].label,
            };
            this.options.push(a);
            this.diseases.push(a);
            for (
              var p = 0;
              p < this.tableData[i].children[j].children.length;
              p++
            ) {
              let a = {
                value: this.tableData[i].children[j].children[p].id,
                label: this.tableData[i].children[j].children[p].label,
              };
              this.diseases.push(a);
            }
          }
    ...
    addDisease() {
      getCategory(
        "/api/category/checkDiseaseName/" + this.addDiseaseParam.firstDisease
      ).then((response) => {
        if (response.code == 200) {
          ...
          postRequest("api/category/addCategory", this.addDiseaseParam).then(
            (res) => {
              console.log(res);
              if (res.code == 200) {
    ...
    updateDisease() {
      console.log(this.updateDiseaseParam);
      postRequest("api/category/updateCategory", this.updateDiseaseParam).then(
        (res) => {
          console.log(res);
          if (res.code == 200) {
    ...
    deleteDisease(row) {
      ...
      if (row.tableNum0 == 0 && row.tableNum1 == 0 && row.tableNum2 == 0) {
        ...
        console.log(data);
        postRequest("api/category/deleteCategory", this.deleteDis).then(
          (res) => {
            console.log(res);
            if (res.code == 200) {
