<template>
  <div class="Box">
    ...
    <div>
      <div class="top">
        <span class="lineStyle">▍</span><span class="featureTitle">想研究哪些因素与疾病的关系：</span>
        <el-checkbox v-model="checkAll_target" @change="
            (val) => {
      ...
      <div v-if="peopleFeatures.length > 0">
        ...
        <el-checkbox-group v-model="targetFeature" @change="changeBox_target()">
          <el-checkbox v-for="item in peopleFeatures" :label="item" :key="item" border><el-popover placement="top-end"
              width="150" :title="item" trigger="hover" :content="getDeficiency(item)" :open-delay=200>
      ...
      <div v-if="physiologicalFeatures.length > 1">
        ...
        <el-checkbox-group v-model="targetFeature" @change="changeBox_target()">
          <el-checkbox v-for="item in physiologicalFeatures" :label="item" :key="item" border><el-popover
              placement="top-end" width="150" :title="item" trigger="hover" :content="getDeficiency(item)"
              :open-delay=200>
              <span slot="reference">{{ item }}</span>
            </el-popover></el-checkbox>
        </el-checkbox-group>
      ...
      <div v-if="socialFeatures.length > 1">
        ...
        <el-checkbox-group v-model="targetFeature" @change="changeBox_target()">
          <el-checkbox v-for="item in socialFeatures" :label="item" :key="item" border><el-popover placement="top-end"
              width="150" :title="item" trigger="hover" :content="getDeficiency(item)" :open-delay=200>
              <span slot="reference">{{ item }}</span>
            </el-popover></el-checkbox>
        </el-checkbox-group>
    ...
    <div>
      <div class="top">
        <span class="lineStyle">▍</span><span class="featureTitle">参与运算的疾病：</span>
        <el-checkbox v-model="checkAll_1" @change="
            (val) => {
      ...
      <div>
        <el-checkbox-group v-model="computeFeatures" @change="changeBox_1()">
          <el-checkbox v-for="item in labelList" :label="item" :key="item" border><el-popover placement="top-end"
              width="150" :title="item" trigger="hover" :content="getDeficiency(item)" :open-delay=200>
              <span slot="reference">{{ item }}</span>
            </el-popover></el-checkbox>
        </el-checkbox-group>
    ...
    <div>
      <div class="top">
        <span class="lineStyle">▍</span><span class="featureTitle">已知相关疾病：</span>
        <el-checkbox v-model="checkAll_2" @change="
            (val) => {
    ...
    <div>
      <div calss="top">
        <span class="lineStyle">▍</span><span class="featureTitle">请根据特征重要性拖动滑块设置奖励程度</span>
      </div>
    </div>
